...
#     sleep(0.1)
#     A.apply(A.compute_next_action())
#     total_turn += 1
    
# print(f"TOTAL TURN = {total_turn}")
...
# depth_limit = 10
# game_counter = 0
# for a,d in zip(game_list, depth_list):
#     game_counter += 1
...
#         game_list += children_list
#         depth_list += [d+1, ] * len(children_list)
...
#     else:
#         children_list = a.project_all()
#         game_queue = children_list + game_queue
#         depth_queue = [d+1, ] * len(children_list) + depth_queue
   
### PLOT
# plt.close('all')

# d_arr = np.array(depth_list)

# depth_range = np.unique(d_arr)
# depth_pop_range = np.zeros(depth_range.shape)
# for i, d in enumerate(depth_range):
#     depth_pop_range[i] = np.count_nonzero(d_arr==d)
    
# fig, axes = plt.subplots(ncols=2)

# ax = axes[0]
# ax.plot(depth_range, depth_pop_range, marker='.')

# ax = axes[1]
# day_range = np.arange(20)
# for depth, day_arr in new_day_dict.items():
#     ax.plot(day_range, day_arr, marker='.')
# ax.set_xlabel('New Day')
# ax.set_ylabel('Number of Game')    


# =============================================================================
# Conclusion of first insight:
# it becomes too long to project on more than 4 days
# The idea now is too analyse the characteristics of the all the game state
# at day 4, and select the best strategies, upon which we can project 4 days later
# eventually reaching the day 25, with a good strategy
# We can expect some difficulties choosing "good" strategies as we approach the later days
# the main goal is to maximized the score on day 25 = good criterion for later good strategies
# for earlier days, a good criterion is to maximize the Sun Points income, with older trees
# =============================================================================

# =============================================================================
# Analysiss of all the game state at day 4
# =============================================================================
### Depth first algorithm stopping at (day=4, turn=0)
day_limit = 4
game_counter = 0
...
for game in gs4_list:
    metric = []
    # my_sun
    metric.append(game.my_sun)
    # my_score
    metric.append(game.my_score)
    # number of trees size 0
    metric.append(game.trees[0])    
    # number of trees size 1
    metric.append(game.trees[1])
    # number of trees size 2
    metric.append(game.trees[2])       
    # number of trees size 3
    metric.append(game.trees[3]) 
